-- =============================================================================
-- v2_0042 — keep the main lifts, vary the accessories, and end the block
--
-- Two spec sections, one mechanism.
--
-- §8.3 continuity: "an exercise the user has logged recently, which still fits
-- the new plan's muscle budget and filters, keeps its slot". Until now every
-- rebuild reshuffled everything, so changing your training days could silently
-- take away the bench press you were four weeks into.
--
-- §8.4 variation: at a block boundary, "keep main lifts, vary 1-2 accessories".
--
-- These are the same comparison read in opposite directions: recently trained
-- compounds sort FIRST (keep them), recently trained isolation sorts LAST
-- (swap it out). One ORDER BY term does both.
--
-- Also lands §8.4's block length on programs.ends_on, which has existed and
-- gone unused. Beginners get no end date on purpose — for them linear
-- progression is the block, and its stall is the boundary that §8.1 promotes
-- on.
-- =============================================================================
