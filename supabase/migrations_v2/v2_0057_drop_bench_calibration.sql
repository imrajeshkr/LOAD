-- =============================================================================
-- v2_0057 — stop overriding bench with a 5x5 calibration ramp
--
-- The onboarding step that asked "how heavy is your bench?" is being removed:
-- it demanded a number most people were guessing at, to set a load the app
-- corrects after the first set anyway.
--
-- Its absence is mostly harmless. `has_benched` is simply false for everyone
-- now, which already means empty bar on barbell work and 40% of the catalogue
-- default elsewhere — the conservative start we want when we know nothing, and
-- one that linear progression closes within a couple of sessions.
--
-- What is NOT harmless is the other thing that flag did: it forced bench to
-- 5 sets of 5 as a calibration protocol. That made sense as a one-off probe
-- for a number we had asked about. With nobody asked, it would apply to
-- everyone forever, silently overriding the rep range their GOAL earns them —
-- a lifter training for fat loss would get 5x5 bench instead of 10-15, for no
-- reason they could see.
--
-- So the ramp goes and the conservative starting load stays.
-- =============================================================================
...
begin
  ...
  for v_i in 1 .. v_daycount loop
    ...
    for v_slot_i in 1 .. v_slots loop
      ...
      values (v_ordinal, v_ex.exercise_id, v_ex.mechanic, v_ex.uses_barbell,
              v_sets, v_rx.rep_low, v_rx.rep_high,
              v_kg, v_rest);
  ...
  update program_day_exercises pde
   ...
   where s.user_id = p_user_id
     and (s.expires_on is null or s.expires_on >= v_start)
     and (s.expires_on is null or s.expires_on >= v_start)
