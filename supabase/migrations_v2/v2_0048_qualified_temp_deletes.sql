-- =============================================================================
-- v2_0048 — the four unqualified DELETEs that broke onboarding
--
-- Onboarding failed at "Build my week" with:
--   PostgrestException(message: DELETE requires a WHERE clause, code: 21000)
--
-- bootstrap_user_program clears its scratch tables between runs with
-- `delete from _cand;` and three more like it. Unqualified DELETE is refused
-- for the role the app connects as, so the generator raised before it could
-- build anything. Adding `where true` satisfies the guard and changes nothing
-- about what the statements do.
--
-- WHY THIS SURVIVED SO MUCH TESTING
-- Every SQL suite, and every reproduction of this failure, connects as the
-- table owner over the pooler — where the guard does not apply. bootstrap
-- therefore succeeded in all 18 suites and in four separate hand-built
-- reproductions, including ones that set role to `authenticated`, because
-- SET ROLE does not reproduce the app's connection. The bug was only ever
-- visible from a real client.
--
-- Introduced by v2_0029, which added the scratch tables. It has been broken
-- for every new signup since — existing users were unaffected because nothing
-- regenerates a working plan, which is exactly the blind spot v2_0045 was
-- written to close.
-- =============================================================================
