-- =============================================================================
-- v2_0029 — volume-driven plan generation
--
-- Implements spec §6 (stages 1-5) and closes the §14 limitation.
--
-- THE INVERSION
-- The old generator picked one exercise per *pattern* per day, ordered by
-- default_rep_low. A day therefore had a fixed number of seats, muscles raced
-- for them, and small muscles — which carry the highest rep ranges — lost that
-- race deterministically. Side Delt, Rear Delt and Calves were dropped from
-- every Upper/Lower and Full-body plan, every time.
--
-- This version allocates a *weekly set budget per muscle* first, then fills
-- days from whichever muscle is furthest below its target. A muscle no longer
-- competes for a seat in one session; it earns sets across the week. Coverage
-- is a property of the algorithm rather than a lucky ordering.
--
-- WHAT ELSE CHANGES
--   · `core` joins the day patterns. No split included it before, so Abs were
--     never trained under any plan despite three Core ab exercises existing.
--   · One program_day per *training day* instead of one per day-type. A 4-day
--     Upper/Lower user used to get 2 days cycled twice — identical sessions.
--     Now Upper A and Upper B are distinct rows, which is what lets the budget
--     rotate emphasis across the week.
--   · Reps and rest come from the goal (spec §6 stage 4). `goal` was collected
--     at onboarding and then ignored; build_muscle and strength produced
--     byte-identical plans.
--   · Set counts come from mechanic and goal rather than the ordinal, and are
--     deliberately NOT derived from the budget — see the note at the call site.
--   · Compound-vs-isolation ordering now reads `mechanic` (v2_0028) instead of
--     using rep range as a proxy for it.
-- =============================================================================

-- ── Stage 1 helpers ─────────────────────────────────────────────────────────

-- Hard sets per muscle per week. Experience sets the ceiling; goal shifts it.
-- Strength trades volume for load, general_health sits at the low end, and the
-- hypertrophy-ish goals take the full budget.
create or replace function plan_weekly_sets(
  p_exp experience_level, p_goal training_goal
  ...
  select greatest(6, round(
    ...
    * (case p_goal
         when 'strength'       then 0.8
         when 'general_health' then 0.85
         else                       1.0
       end)
  )::int);
$$;

-- Rep range and rest, per goal (spec §6, stage 1 table).
create or replace function plan_goal_prescription(p_goal training_goal)
returns table (rep_low int, rep_high int, rest_compound int, rest_isolation int)
language sql immutable as $$
  select
    (case p_goal when 'strength' then 3  when 'lose_fat' then 10
                 when 'general_health' then 8 when 'recomposition' then 8
                 else 6 end)::int,
    (case p_goal when 'strength' then 6  when 'lose_fat' then 15
                 when 'general_health' then 15 when 'recomposition' then 12
                 else 12 end)::int,
    (case p_goal when 'strength' then 180 when 'lose_fat' then 90 else 120 end)::int,
    (case p_goal when 'strength' then 150 when 'lose_fat' then 60 else 90 end)::int;
$$;


-- ── The generator ───────────────────────────────────────────────────────────

create or replace function public.bootstrap_user_program(
  p_user_id uuid, p_bench_start_kg numeric default null
) returns uuid
-- pg_temp is named explicitly and LAST. This function is SECURITY DEFINER and
-- creates temp tables; without pg_temp in the path Postgres searches it first
-- implicitly, which would let a caller pre-create `_cand` or `_budget` in their
-- own session and have the definer-rights body resolve to it.
