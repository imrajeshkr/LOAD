-- =============================================================================
-- v2_0050 — do not hand someone a split their gym cannot fill
--
-- Found by supabase/tests/v2_matrix_sweep.sql on its first run: 20 of 275 plan
-- shapes produced a day containing a single exercise. Every one of them was
-- bodyweight-only combined with Push/Pull/Legs, because the Core catalog has
-- exactly ONE bodyweight pull lift a beginner can do — Chin-Up — so "Pull day"
-- was one exercise long.
--
-- The split was honoured whenever the lifter chose it explicitly, with no check
-- that anything existed to fill it. Spec §6 stage 3 asks selection to degrade
-- gracefully instead of emitting a broken plan, so an unfillable split now
-- falls back to full body — which pools every pattern, and is what someone
-- training with nine available exercises should be doing regardless.
--
-- Deliberately conservative: the fallback only triggers when a day-type cannot
-- field three lifts. A thin-but-workable split is left alone.
-- =============================================================================
