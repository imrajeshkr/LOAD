-- =============================================================================
-- v2_0049 — a bigger week arrives over two weeks, however it was asked for
--
-- §8.3 item 3 ("deload on structural change") was the one part of §8 never
-- built. v2_0040 added the ramp and wired it to level changes; v2_0043 wired
-- it to block rollovers and layoff returns. Nothing wired it to the path a
-- lifter actually triggers on purpose: changing split or training days in
-- Profile, which calls generateProgram() straight through to
-- bootstrap_user_program.
--
-- So three days to six — the exact example §8.3 uses for how people get hurt —
-- landed the entire volume increase in one week.
--
-- Putting the check inside bootstrap_user_program covers every caller at once,
-- including the two that already set the ramp themselves (setting it twice is
-- harmless; the later date wins and both are +7 days).
-- =============================================================================
